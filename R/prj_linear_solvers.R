...
as_node.r2ogs6_linear_solver <- function(obj) {
    ...
    linear_solver_node <- list(nonlinear_solver = structure(list()))

    linear_solver_node <- add_children(nonlinear_solver_node, list(name = obj$name,
                                                                   eigen = obj$eigen,
                                                                   lis = obj$lis,
                                                                   petsc = obj$petsc))

    return(linear_solver_node)
}